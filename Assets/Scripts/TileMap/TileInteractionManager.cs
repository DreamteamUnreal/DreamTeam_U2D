...
public class TileInteractionManager : MonoBehaviour
{
	...
	{
		return groundTilemap != null ? groundTilemap.WorldToCell(worldPosition) : Vector3Int.zero;
	}
	...
	{
		return groundTilemap != null ? groundTilemap.GetCellCenterWorld(cellPosition) : Vector3.zero;
	}
	...
	{
		return targetTilemap != null && targetTilemap.GetTile(cellPosition) == targetTile;
	}
	...
	public List<Vector3Int> GetNeighborCells(Vector3Int cell)
	{
		...
		for (int x = -1; x <= 1; x++)
		{
			for (int y = -1; y <= 1; y++)
			{
				...
				Vector3Int neighborCell = new(cell.x + x, cell.y + y, cell.z);
