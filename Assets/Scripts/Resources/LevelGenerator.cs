...
public class LevelGenerator : MonoBehaviour
{
	...
	private readonly string[] levelLayout = new string[]
	{
...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	private void SetupTilemaps()
	{
		...
		if (gridObject == null)
		{
			...
			_ = gridObject.AddComponent<Grid>();
		}

		// Create ground tilemap
		if (groundTilemap == null)
		{
			GameObject tmObject = new("GroundTilemap");
			tmObject.transform.SetParent(gridObject.transform);
			groundTilemap = tmObject.AddComponent<Tilemap>();
			_ = groundTilemap.gameObject.AddComponent<TilemapRenderer>();
		...
		{
			GameObject tmObject = new("ObstacleTilemap");
			tmObject.transform.SetParent(gridObject.transform);
			obstacleTilemap = tmObject.AddComponent<Tilemap>();
			_ = obstacleTilemap.gameObject.AddComponent<TilemapRenderer>();
		...
		{
			GameObject tmObject = new("InteractableTilemap");
			tmObject.transform.SetParent(gridObject.transform);
			interactableTilemap = tmObject.AddComponent<Tilemap>();
			_ = interactableTilemap.gameObject.AddComponent<TilemapRenderer>();
	...
	private void SpawnPlayerAndAnimals()
	{
		...
		for (int y = 0; y < levelHeight; y++)
		{
			...
			for (int x = 0; x < levelWidth; x++)
			{
				...
				{
					_ = Instantiate(playerPrefab, worldPos, Quaternion.identity);
					playerSpawned = true;
				...
				{
					_ = Instantiate(squirrelPrefab, worldPos, Quaternion.identity);
				}
				// Example: Spawn an apple if it's a separate GameObject
				else if (tileChar == 'A' && applePrefab != null && Random.value < 0.5f) // 50% chance on 'A'
				{
					_ = Instantiate(applePrefab, worldPos, Quaternion.identity);
				}
			}
		}
		if (!playerSpawned)
		{
			Debug.LogWarning("LevelGenerator: Player spawn point 'P' not found or player prefab not assigned!");
		}
