...
namespace HappyHarvest // Add the HappyHarvest namespace
{
    public class PieScoreManager : MonoBehaviour
    {
        ...
        private void UpdateUI()
        {
            ...
            else Debug.LogWarning("PieScoreManager: currentPiesText is null.");

            if (currentRoundText != null)
                currentRoundText.text = $"{currentRounds}";
            else Debug.LogWarning("PieScoreManager: currentRoundText is null.");

            if (numPiesText != null)
                numPiesText.text = $"{piesRequired}";
            else Debug.LogWarning("PieScoreManager: numPiesText is null.");

            if (numRoundsText != null)
                numRoundsText.text = $"{roundNum}";
            else Debug.LogWarning("PieScoreManager: numRoundsText is null.");

            if (playerScoreText != null)
                playerScoreText.text = $"{playerScore}";
            else Debug.LogWarning("PieScoreManager: playerScoreText is null.");

            if (highScoreText != null)
                highScoreText.text = $"{highScore}";
            else Debug.LogWarning("PieScoreManager: highScoreText is null.");
        }
    }
}