...
namespace HappyHarvest
{
	public static class SaveSystem
	{
		...
		private static SaveData s_CurrentData = new();

		// Runtime lookup for scene-specific data. This is NOT directly serialized.
		private static readonly Dictionary<string, SceneData> s_ScenesDataLookup = new();
		...
		public static void Load()
		{
			...
			{
				foreach (SceneData sceneData in s_CurrentData.AllScenesData)
				{
		...
		public static void SaveCurrentSceneDataToLookup()
		{
			...
			TerrainDataSave terrainSaveData = new();
			List<InteractiveObjectSaveData> interactiveObjectSaveDatas = new();
			...
			foreach (InteractiveObject obj in Object.FindObjectsOfType<InteractiveObject>())
			{
		...
		public static void LoadCurrentSceneDataFromLookup()
		{
			...
			if (s_ScenesDataLookup.TryGetValue(currentSceneName, out SceneData loadedSceneData))
			{
				// Load Terrain Data
				if (GameManager.Instance.Terrain != null)
				{
				...
				foreach (InteractiveObjectSaveData loadedObjectData in loadedSceneData.InteractiveObjectsData)
				{
