...
namespace HappyHarvest
{
    ...
    public class InventorySystem : MonoBehaviour
    {
        ...
        public Item EquippedItem
        {
            get
            {
                if (EquippedItemIdx >= 0 && EquippedItemIdx < Entries.Length && !Entries[EquippedItemIdx].IsEmpty())
                {
                    return Entries[EquippedItemIdx].Item;
                }
                return null;
            }
        }
        ...
        {
            if (itemToCheck == null || requiredQuantity <= 0) return false;
        ...
        public bool CanFitItem(Item item, int count)
        {
            ...
            foreach (var slot in Entries)
            {
                if (slot.Item == item && slot.StackSize < item.MaxStackSize)
                {
                    ...
                    if (remainingCount <= 0) return true; // All items fit
                }
            }
